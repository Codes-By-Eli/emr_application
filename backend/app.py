...
@app.route("/get_progress_numbers", methods=['GET'])
def get_progress():
    table = "progress_note"
    columns = ["progress_id"]
...
@app.route("/get_discharge_numbers", methods=['GET'])
def get_discharge():
...
def get_eval():
    try:
        ...
        body = {
            ...
            "est_len": init_eval[0][25],
            "overall_goals": init_eval[0][26],
            "short_term_goals": init_eval[0][27]
            }
        body2 = {
            "fim_eval_id": fim_eval_id[0][0],
            "vital_id": vitals[0][0],
...
@app.route("/get_chosen_progress", methods=['POST'])
def get_progress_choice():
    try:
        data = request.json
        table = "progress_note"
        columns = ["*"]
        condition_column = "progress_id"
        select_condition = f"\"{data['record']}\""

        progress_note = database.perform_select(table, columns, condition_column,select_condition)

        table = "clients"
        columns = ["*"]
        condition_column = "client_id"
        select_condition = f"\"{progress_note[0][1]}\""
        client = database.perform_select(table, columns, condition_column, select_condition)

        body = {
            "med_num": progress_note[0][0],
            "name": client[0][1] + ' ' + client[0][2],
            "DOB": client[0][3],
            "sex": client[0][4],
            "diagnosis": progress_note[0][4],
            "precautions": progress_note[0][5],
            "contraindications": progress_note[0][6],
            "summaryOfServices": progress_note[0][7],
            "clientPerformace": progress_note[0][8],
            "planOrReccomendations": progress_note[0][9],
            "therapistSignature": progress_note[0][10],
            "date": progress_note[0][11],
            "units": progress_note[0][12]
        }
        response = {
            "data": body
        }, 200
...
def submit_discharge():
    ...
    try:
        ...
        if(discharge_eval['msg'] != "success"):
            raise Exception()
    except:
        try:
            del values[0]
            del params[0]
            key_column = "medical_record_id"
            key_value = data['med_num']
            database.perform_update(table,params,values,key_column,key_value)
        except:
...
@app.route("/get_chosen_discharge", methods=["POST"])
def chosen_discharge():
    try:
        data = request.json
        table = "discharge_evaluation"
        columns = ["*"]
        condition_column = "medical_record_id"
        select_condition = f"\"{data['record']}\""
        disc_eval = database.perform_select(table,columns,condition_column,select_condition)
        
        table = "discharge_fim_scores"
        columns = ["*"]
        condition_column = "discharge_fim_id"
        select_condition = f"\"{disc_eval[0][3]}\""
        disc_fim_scores = database.perform_select(table,columns,condition_column, select_condition)
        ...
        if(disc_eval[0][2] != ''):
            ...
            columns = ["*"]
            condition_column = "medical_record_id"
            select_condition = f"\"{disc_eval[0][2]}\""
            init_eval = database.perform_select(table, columns, condition_column, select_condition)
        ...
        print("Adding discharge fim stuff")
        disc_results = {
            "med_num": disc_eval[0][0],
            "date": disc_eval[0][6],
            "discharge_transfer": disc_eval[0][16],
            "disc_assess": disc_eval[0][18],
            "discharge_recommendations_referrals": disc_eval[0][24],
            "course_of_rehab": disc_eval[0][22],
            "eat_disc": disc_fim_scores[0][2],
            "groom_disc": disc_fim_scores[0][3],
            "bath_disc": disc_fim_scores[0][4],
            "upper_disc": disc_fim_scores[0][5],
            "lower_disc": disc_fim_scores[0][6],
            "toilet_disc": disc_fim_scores[0][7],
            "toilet_transfer_disc": disc_fim_scores[0][8],
            "shower_transfer_disc": disc_fim_scores[0][9],
            "tub_transfer_disc": disc_fim_scores[0][10]
        }
        response.update(disc_results)
        print(f"Passed adding disc values")
...
def insert_initial(data):
    ...
    try: 
        ...
        print("before client insertion")
        client_id = database.perform_insert(table,params,values)
        
        print("Client is fine")
        ...
        try:
            ...
            del params[0]
            key_column = "medical_record_id"
            key_value = data['allValues']['med_num']
            print("before update")
            database.perform_update(table,params,values,key_column, key_value)
            print("after update")
            return insertion
...
def submit_progress_eval():
    data = request.json
    #print(data)
    data['record_number'] = data['med_num']
    ...
    try: 
        ...
        print("before values")
        print(f"Client ID: {client_id['last_id']}")
        values = [
            data['med_num'],
            client_id['last_id'],
            user_id,
    ...
    if(not sql_conversion or not pdf_conversion or not json_conversion):
        response = {
            "msg": data
        }, 400
    else:
...
if __name__ == '__main__':
    if(os.path.exists("./emr_database.db")):
        #database = SQL_Interaction("./backend/SQL/emr_database.db")
        database = SQL_Interaction("emr_database.db")
        database.create_connection()
        print("Connecting to an existing database..")
    else:
        print("Attempting to make database...")
        try:
            #database = SQL_Interaction("./backend/SQL/emr_database.db")
            database = SQL_Interaction("emr_database.db")
            database.create_connection()
